...
def assement(TP,FN,FP,TN):
    ...
    return correct_rate,sensitivity,specificity,ppc,npv,F1_score
...
for j in range(0,10):
    ...
    for train_index, test_index in skf:
        ...
        clf = svm.LinearSVC(class_weight={1:8,0:1})
        clf.fit(train_data, train_y)
        pred = clf.predict(test_data)
        TP,FN,FP,TN = evulate(test_y,pred)
        # print TP,FN,FP,TN
        correct_rate, sensitivity, specificity, ppc, npv, F1_score = assement(TP, FN, FP, TN)
        end_time = time.time()
